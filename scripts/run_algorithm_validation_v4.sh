...
OUTDIR="$PROJECT_DIR/test-results/algorithm/$TIMESTAMP"
...
record_summary "Algorithm validation v4 summary"
...
if ! command -v cargo >/dev/null 2>&1; then
    record_summary ""
    record_summary "cargo is not available in this environment."
    exit 127
fi

cd "$PROJECT_DIR" || exit 1

run_step "01_cargo_test_top_tier_v4" cargo test --test top_tier_v4
run_step "02_cargo_test_basic" cargo test --test basic
run_step "03_cargo_test_integration" cargo test --test integration
run_step "04_cargo_test_security" cargo test --test security
run_step "05_cargo_test_regression" cargo test --test regression
run_step "06_cargo_test_real_world" cargo test --test real_world
